...
mac80211_hostapd_setup_base() {
	...
	if [ -n "$iface" ]
	then
		cfgfile="/var/run/hostapd-"$iface"-$phy.conf"
		macfile="/var/run/hostapd-"$iface"-$phy.maclist"
	else		
	...
	if [ -n "$iface" ] 
		then
			ctrl_interface=/var/run/hostapd-$iface-$phy 
		else
...
mac80211_hostapd_setup_bss() {
	...
	if [ -f "$cfgfile" ]; then	
		...
		mac80211_hostapd_setup_base "$phy" "$ifname" "$iface"		
	...
	hostapd_set_bss_options hostapd_cfg "$vif" "$iface"
...
enable_mac80211() {
	...
	# As long as this is not used by the create n aps script, no changes are required
